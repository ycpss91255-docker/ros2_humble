...
> **TL;DR** — 一鍵建置 ROS 2 Humble 容器化開發環境。自動偵測 UID/GID/GPU，支援 X11 GUI 轉發，多階段建置含 smoke test 驗證。
>
> ```bash
> ./build.sh && ./run.sh
> ```
...
- ROS 環境：`ROS_DISTRO`、`setup.bash` 可 source、`ros2` CLI 可用
- Dev tools：`colcon`、`python3`、`git` 可用
