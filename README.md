...
> **TL;DR** — One-command ROS 2 Humble containerized dev environment. Auto-detects UID/GID, supports X11 GUI forwarding, multi-stage build with smoke test verification.
>
> ```bash
> ./build.sh && ./run.sh
> ```
...
- **X11 forwarding**: supports GUI applications (RViz2, Terminator, etc.)
...
Full dev environment with colcon, tmux, terminator, vim, git, etc.
...
Minimal image with only essential ROS 2 packages.
...
|:--------:|------|-------------|--------|
| 1 | Last directory matches `docker_*` → strip prefix | `/home/user/docker_ros2_humble` | `ros2_humble` |
| 2 | Scan path (right→left) for `*_ws` → use prefix | `/home/user/ros2_humble_ws/docker_ros2_humble` | `ros2_humble` |
...
| `sys` | `ros:humble-ros-base-jammy` | OS base: user/group, locale, timezone |
| `base` | `sys` | Common dev tools + colcon + ros2cli (apt) |
...
| `runtime` | `runtime-base` | Adds required ROS 2 packages |
